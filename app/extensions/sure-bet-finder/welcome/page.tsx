...
function SureBetFinderWelcomeInner() {
  ...
  useEffect(() => {
    // Update eyebrow + update banner based on query params
    ...
    if (reason === "update" && prev) {
      ...
      if (bannerText)
        bannerText.textContent =
          "You just updated from v" +
          prev +
          ". New: faster scans, in-extension feedback, polished loading.";
    } else {
  ...
  function handleGtagLoaded() {
    ...
    // Fire Google Ads conversion ONLY on first install, never on update.
    if (reason === "install" && !conversionFired.current) {
      ...
      console.log(
        '[Polymarket Welcome] Conversion NOT fired (reason is not "install").'
      );
    }
  }
    ...
    <>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sbf-headline {
          ...
          opacity: 0;
          animation: sbfFadeUp 0.8s cubic-bezier(0.22,1,0.36,1) 0.3s forwards;
        }
        .sbf-headline .accent { color: var(--accent); font-style: italic; font-weight: 600; }
        .sbf-sub {
          font-size: 16px; color: var(--text-muted);
          max-width: 52ch; margin-bottom: 22px; opacity: 0;
          animation: sbfFadeUp 0.8s cubic-bezier(0.22,1,0.36,1) 0.4s forwards;
        }
        .sbf-actions {
          display: flex; gap: 12px; align-items: center; opacity: 0;
          animation: sbfFadeUp 0.8s cubic-bezier(0.22,1,0.36,1) 0.5s forwards;
        }
        .sbf-asset {
          ...
          animation: sbfFadeUp 0.9s cubic-bezier(0.22,1,0.36,1) 0.5s forwards;
        }
        .sbf-asset-title {
        ...
        .sbf-asset-row:nth-child(1) .sbf-bar-fill { animation-delay: 0.8s; --w: 0.98; }
        .sbf-asset-row:nth-child(2) .sbf-bar-fill { animation-delay: 0.95s; --w: 0.97; }
        .sbf-asset-row:nth-child(3) .sbf-bar-fill { animation-delay: 1.1s; --w: 0.965; }
        .sbf-asset-row:nth-child(4) .sbf-bar-fill { animation-delay: 1.25s; --w: 0.95; }
        .sbf-asset-row:nth-child(5) .sbf-bar-fill { animation-delay: 1.4s; --w: 0.92; }
        ...
        .sbf-section {
          margin-top: 64px; opacity: 0;
          animation: sbfFadeUp 0.8s cubic-bezier(0.22,1,0.36,1) 0.7s forwards;
        }
        .sbf-section-label {
        ...
        .sbf-feature:hover {
          ...
          box-shadow: 0 1px 2px rgba(15,23,42,0.05), 0 8px 24px -8px rgba(15,23,42,0.1);
        }
        .sbf-feature-icon {
        ...
        .sbf-feature-title { font-size: 15px; font-weight: 600; margin-bottom: 6px; letter-spacing: -0.01em; }
        ...
        .sbf-pricing {
          ...
          box-shadow: var(--shadow-elev);
          opacity: 0;
          animation: sbfFadeUp 0.9s cubic-bezier(0.22,1,0.36,1) 1.1s forwards;
        }
        @media (max-width: 880px) {
          .sbf-pricing { grid-template-columns: 1fr; gap: 32px; padding: 32px 24px; }
        }
        .sbf-pricing-badge {
        ...
        .sbf-price-num { font-size: 56px; font-weight: 800; letter-spacing: -0.04em; color: var(--text); line-height: 1; }
        ...
        .sbf-feature-list li { display: flex; align-items: flex-start; gap: 12px; font-size: 14px; color: var(--text); }
        .sbf-check {
        ...
        .sbf-feature-list strong { font-weight: 600; color: var(--text); }
        .sbf-feature-list .muted { color: var(--text-muted); font-weight: 400; }

        .sbf-onboarding { display: none; margin-top: 64px; }
        .sbf-onboarding.active { display: block; opacity: 0; animation: sbfFadeUp 0.7s cubic-bezier(0.22,1,0.36,1) 0.15s forwards; }
        ...
        .sbf-onboarding-tip strong { color: var(--text); font-weight: 600; }
        ...
        .sbf-foot {
          ...
          font-size: 13px; color: var(--text-dim); opacity: 0;
          animation: sbfFadeIn 0.8s ease 1.4s forwards;
        }
      ...
      <div className="sbf-wrap">
        <div className="sbf-page">
          ...
          {/* Onboarding — install only */}
          <div className="sbf-onboarding" id="onboardingSection">
